...
export const Timeline = ({ title, data }) => {
  ...
  return (
    <div className="w-full bg-transparent" ref={containerRef}>
      <div className="max-w-7xl py-20">
        <h2 className="text-lg md:text-4xl mb-4 max-w-4xl font-semibold">
          <span className="bg-primary p-2 rounded-xl">{title}</span>
        </h2>
      </div>
      <div ref={ref} className="relative w-full mx-auto pb-20">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-start pt-10 md:pt-40 md:gap-30"
          >
